...
function init() {
    ...
    const capitalRegions = ["AT13", "BE10", "BG41", "CY00", "CZ01", "DE30", "EL30", "DK01", "EE00", "ES30", "FI1B",
        "FR10", "HR04", "HU11", "IE06", "LU00", "LV00", "MT00", "NL32", "ITI4", "LT01", "PL91", "PT17", "SI04", "SK01", "RO32", "SE11", "UKI4"
    ];
                    ...
                    d3.json("assets/data/land.json", function (land) {
                            d3.json("assets/data/capitals-gdppps17.json", function (capitals) {
                                    ...
                                    function handleStepEnter(step) {
                                        //Track scrolling
                                        tracker.send({
                                            category: `${step.index}-${step.direction}`,
                                            action: 'scroll'
                                        });
                                        ...
                                        if (step.index == 4 && step.direction == "down") {
                                            ...
                                            countries.transition().duration(2000)
                                                ...
                                                .attrTween("d", function (d) {
                                                    return toRect(d3.select(this).attr("d"), devLinearScale(+d.properties.gdppps17) - 6, countryScale(d.properties.CNTR_CODE) -
                                                        6, 12, 12);
                                                });
                                            regions
                                                .on("mouseover", function (d) {
                                                    ...
                                                    tooltip.html(d.properties.NUTS_NAME + "<br/>" + d.properties.gdppps17).style("left", (d3.event.pageX + 10) + "px")
                                                        .style("top", (d3.event.pageY - 28) + "px");
                                                    highlightRegions([d.properties.NUTS_ID])
                                                })
                                                .transition().duration(2000).delay((d, i) => i * 20)
                                                .attrTween("d", function (d) {
                                                    return toCircle(d3.select(this).attr("d"), devLinearScale(+d.properties.gdppps17),
                                                        countryScale(d.properties.CNTR_CODE), 6);
                                                });
                                            ...
                                            let ticksYAxis = d3.selectAll(".y-axis .tick text")
                                                ...
                                                }).style("opacity", 0).transition().delay(3000).duration(1000).style("opacity", 1);
                                            let
                                                averageTwo = mapOne.insert("g", "path.region").attr("id", "average-two").attr("transform",
                                                    `translate(${devLinearScale(100)}, 0)`);
                                            averageTwo.append("line").attr("x1", 0).attr("x2", 0).attr("y1",
                                                margin.top).attr("y2", height - 32).attr("class", "average-line-two").style("opacity",
                                                0).transition().delay(3000).duration(1000).style("opacity", 1);
                                        }
                                        if (step.index == 3 && step.direction == "up") {
                                            ...
                                            d3.selectAll(".y-axis").transition().duration(2000).style("opacity", 0);
                                            countries.style("opacity", 1).style("fill-opacity", 0);
                                            ...
                                            scaleLegendCellsMap();
                                            eucaps.transition().delay(1000).duration(2000).attr("cx", (d) => projection(d.geometry.coordinates)[0])
                                                .attr("cy", (d) => projection(d.geometry.coordinates)[1]);
                                            countries
                                                .transition().duration(2000)
                                                .delay((d, i) => i * 200)
                                                .attrTween("d", function (d) {
                                                    return fromRect(devLinearScale(+d.properties.gdppps17), countryScale(d.properties.CNTR_CODE), 6, 6,
                                                        path(d.geometry));
                                                });
                                            regions
                                                .on("mouseover", function (d) {
                                                    ...
                                                    tooltip.html(d.properties.NUTS_NAME).style("left", (d3.event.pageX + 10) + "px").style("top",
                                                        (d3.event.pageY - 28) + "px");
                                                    highlightRegions([d.properties.NUTS_ID])
                                                }).transition().duration(2000)
                                                .delay((d, i) => i * 20)
                                                .attrTween("d", function (d) {
                                                    return fromCircle(devLinearScale(+d.properties.gdppps17), countryScale(d.properties.CNTR_CODE), 6,
                                                        path(d.geometry));
                                                });
                                        }
                                        ...
                                        if (step.index == 7 && step.direction == "down") {
                                            ...
                                            countries.transition().duration(2000)
                                                .attrTween("d", function (d) {
                                                    return toRect(d3.select(this).attr("d"), devLinearScale(+d.properties.gdppps17) - 6,
                                                        countryScale(d.properties.CNTR_CODE) - 6, 12, 12);
                                                });
                                            d3.select("#average")
                                                .transition().duration(2000)
                                                .attr("transform", `translate(${devLinearScale(100)}, 0)`);
                                    ...
                                    const
                                        percapitafundScale = d3.scaleThreshold().domain([25, 50, 75, 125, 200])
                                        .range(["#5B3794", "#8F4D9F", "#B76AA8", "#D78CB1", "#F1B1BE", "#F8DCD9"].reverse()); //RdPu
                                    percapitafundScale.labels = ["<25 €/cap/y", "25-50", "50-75", "75-125", "125-200", ">200 €/cap/y"];
                                    const fundpercgdpScale = d3.scaleThreshold().domain([1, 2, 3, 4, 5])
                                        .range(["#5B3794", "#8F4D9F", "#B76AA8", "#D78CB1", "#F1B1BE", "#F8DCD9"].reverse()); //RdPu
                                    fundpercgdpScale.labels = ["<1% gdp", "1-2", "2-3", "3-4", "4-5", ">5% gdp"];
                                    let
                                        geojsonNUTS2 = topojson.feature(nuts2, nuts2.objects.foo);
                                    let geojsonNUTS0 = topojson.feature(nuts0,
                                        nuts0.objects.foo);
                                    geojsonNUTS0.features = geojsonNUTS0.features.sort(function (x, y) {
                                        return
                                        d3.descending(+x.properties.gdppps17, +y.properties.gdppps17)
                                    }) let
                                        countrycodes = geojsonNUTS0.features.map((country) => country.properties.CNTR_CODE);
                                    let margin = {
                                        "top": 70,
                                        "left": 120,
                                        "bottom": 50,
                                    ...
                                    const mapPadding = 10 + marginTitleLegend; //Scales for
                                    the dotplot
                                    const countryScale = d3.scalePoint().domain(countrycodes).range([margin.top +
                                        marginTitleLegend, height - margin.bottom
                                    ]) const devLinearScale = d3.scaleLinear().domain([20,
                                            d3.max(geojsonNUTS2.features, (d) => +d.properties.gdppps17)
                                        ])
                                    ...
                                    let regions = mapOne.selectAll("path.region")
                                        ...
                                        .on("mouseover", function (d) {
                                            ...
                                            tooltip.html(d.properties.NUTS_NAME).style("left", (d3.event.pageX +
                                                10) + "px").style("top", (d3.event.pageY - 28) + "px");
                                            highlightRegions([d.properties.NUTS_ID])
                                    ...
                                    let
                                        countries = mapOne.selectAll("path.country").data(geojsonNUTS0.features).enter().append("path")
                                        .attr("class", "country").attr("d", path).attr("id", (d) => d.id)
                                        .style("fill", (d) => devscale(d.properties.gdppps17));
                                    ...
                                    //Dot animations
                                    let drawAnimation = function (countrycode, region1ID, region2ID, oldvalue, oldregionID,
                                        countryNameEmoji) {
                                        ...
                                        let
                                            animsvg = d3.select(`.animation-${countrycode}`).attr("width", animWidth).attr("height",
                                                100);
                                        let avg = animsvg.append("g").attr("transform", `translate(${localScale(100)}, 20)`);
                                        avg.append("line").attr("x1", 0).attr("x2", 0).attr("y1", -20).attr("y2", 20)
                                            .style("stroke-width", 1).style("stroke", "#000000");
                                        avg.append('text').attr("x", 0)
                                            .attr("y", 32).text(100).style("text-anchor", "middle").style("fill", "#000000")
                                            .attr("class", "label-outside tk-atlas");
                                        let ninety = animsvg.append("g").attr("transform",
                                            `translate(${localScale(90)}, 20)`);
                                        ninety.append("line").attr("x1", 0).attr("x2", 0)
                                            .attr("y1", -20).attr("y2", 20).style("stroke-width", 1).style("stroke", devscale(89));
                                        ninety.append('text').attr("x", 0).attr("y", 32).text(90).style("text-anchor", "middle").style("fill", d3.color(devscale(89)).darker(1.2).toString()) //d3.color(devscale(89)))
                                            .attr("class", "label-outside tk-atlas");
                                        let seventyfive = animsvg.append("g")
                                            .attr("transform", `translate(${localScale(75)}, 20)`);
                                        seventyfive.append("line")
                                            .attr("x1", 0).attr("x2", 0).attr("y1", -20).attr("y2", 20).style("stroke-width", 1)
                                            .style("stroke", devscale(74));
                                        seventyfive.append('text').attr("x", 0).attr("y", 32)
                                            .text(75).style("text-anchor", "middle").style("fill", devscale(74))
                                            .attr("class", "label-outside tk-atlas");
                                        let
                                            countryRegionsData = geojsonNUTS2.features.filter((reg) => reg.properties.CNTR_CODE ==
                                                countrycode);
                                        animsvg.append("line")
                                        ...
                                        animsvg.selectAll("circle.countryregion").data(countryRegionsData)
                                            ...
                                            .attr("cx", (d) => {
                                                if (d.properties.NUTS_ID == region1ID || d.properties.NUTS_ID == region2ID) {
                                                    ...
                                                    return localScale(d.properties.gdppps17)
                                                }
                                            })
                                            .attr("cy", 20)
                                            .attr("r", 8)
                                        ...
                                        d3.selectAll(".cell rect")
                                            ...
                                            .attr("width", (d, i) => {
                                                if (i < breaks.lenght) {
                                                    ...
                                                    return width - devLinearScale(d);
                                                }
                                            }) d3.selectAll(".cell
                                                text ").transition().duration(2000).style("
                                                opacity ", 0);
                                                d3.select("#average-label").transition().duration(2000).attr("x", devLinearScale(100)) d3.select("#average-mark").transition().duration(2000).attr("x1", devLinearScale(100))
                                                .attr("x2", devLinearScale(100));
                                            }

                                        function scaleLegendCellsMap() {
                                            d3.selectAll(".cell").transition().duration(2000).attr("transform", (d, i) =>
                                                `translate(${i * (width - margin.left - margin.right)/6},0)`);
                                            d3.selectAll(".cell rect")
                                                .transition().duration(2000)
